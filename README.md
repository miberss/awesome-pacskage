# awesome-pacskage

A curated list of packages for [pacskage](https://github.com/miberss/pacskage), a tiny package manager for Skript.

Install any package with `/package install <url>`.

## Packages

- [skript-bindables](https://github.com/miberss/skript-bindables) holds values that run callbacks when they change.
- [easking](https://github.com/miberss/easking) provides easing functions for animation and interpolation.
- [taSK](https://github.com/YouCanDream/taSK) adds a task system for managing content work.
- [skolor](https://github.com/miberss/skolor) hsl, hsv, and color manipulation
- [pacskage-browser](https://github.com/miberss/pacskage-browser) easily browse github pacskages in-game

## Adding a package

Open a pull request adding yours above. It needs a `package.txt` manifest at the repo root (see [pacskage-example](https://github.com/miberss/pacskage-example)).
